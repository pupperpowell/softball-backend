...
export function simulateFielding(ball: BattedBall, team: Team) {
	// evaluate if the ball still needs to be fielded after each of the following stages:
	let live_ball: boolean = true;

	// Who is attempting to catch the ball?
	// (estimateDropZone())

	// Is the catch successful/possible?
	// (catchProbability())

	// If an infielder cannot catch a grounder or line drive, it may roll into the outfield...

	while (live_ball) {
		// Can the current fielder tag anyone out?
		// If not, who should the fielder throw the ball to? (findFieldingTarget())
	}
}
...
function catchProbability(ball: BattedBall, fielder: Player): number {
	// Remember: a "catch" can include ground balls. It's the ability to get a handle on the ball.
	throw new Error("Not implemented")
}

// The ball needs to be thrown to a new position!
// Divide implementation between outfield and infield.
// Refer to OUTFIELD_STRATEGY.md for outfield passing.
/**
 * A fielder throws the ball to another fielder.
 * @param fielder The fielder initiating the pass
 * @return The target fielder
 */
function findFieldingTarget(fielder: Player): FieldingPosition {
	throw new Error("Not implemented");
}

/**
 *
 * @param fielder The fielder attempting the tag
 * @returns If the tag attempt was successful
 * Remember: Force outs can happen on infield hits with a runner heading towards second
 */
function attemptTag(fielder: Player): boolean {
	throw new Error("Not implemented");
}
